...
if __name__ == '__main__':
    ...
    if droid:
        ...
        sonuc = 2
        if sonuc < 3:
            link = "https://cloclo27.cldmail.ru/3Byg1BFddrJtFkoNY7F/G/EgaY/L7Z1R1TqJ/%5BPuzzleSubs%5D%20Okusama%20ga%20Seitokaichou%21%20%2B%21%20%20-%2003%20%5B1080p%5D.mp4?key=02b153e0b800236dd04a9669dbcd295fc5d4e2a6"
            if not link: exit('Geçersiz link')
            if not (link.startswith('http://') or link.startswith('https://')):
                exit('Geçersiz protokol')
            fname = select_name(link)
